...
use strict;
use warnings;
use YAML::Active qw/Dump Reload/;
...
sub make_ticket {
    ...
    my $ticket = $self->delegate->make_obj('test_ticket')->make_whole_ticket(
        %{$self->test_def($testname)->{make_whole_ticket}}
    );
...
sub gen_tx_item_ref {
    ...
    for my $payload_tx ($payload->transactions) {
        ...
        if ($item_spec =~ /^(\w+)\.(\d+)$/) {
            ...
            $payload_tx->transaction->payload_item(eval
                "\$payload->$accessor\->[$index]");
            die $@ if $@;
        }
...
sub run_test {
    ...
    $self->ticket($self->delegate->make_obj('ticket', 
        ticket_no => $self->ticket_no,
    ));
    $self->ticket->read;
...
sub test_expectations {
    my $self = shift;
    # give the objects a chance to prepare themselves for comparison
    $self->ticket->comparable;
}
...
sub plan_ticket_expected_container {
    ...
    while (my ($object_type, $spec) =
        ...
        for my $payload_item (@$spec) {
            for my $exception (ref $payload_item eq 'ARRAY'
                ? @$payload_item
                : $payload_item) {
...
sub check_ticket_rc_status {
    ...
    is($self->ticket->rc, $self->expect->{rc} || $self->delegate->RC_OK,
        $self->named_test('rc'));
...
sub check_ticket_tx {
    ...
    my @tx_status =
        map { $_->transaction->status }
        $self->ticket->payload->transactions;
...
sub compare_exceptions {
    ...
    for my $got_exception (sort { "$a" cmp "$b" }
        ...
        my $expected_exception =
            $expected_exceptions->[$exception_index];
        ...
        $expected_properties{ref} =
            delete $expected_properties{p_ref}
            if $expected_properties{p_ref};

        is_deeply_flex(
            scalar($got_exception->properties_as_hash),
            \%expected_properties,
    ...
    while (defined (my $extra = $expected_exceptions->[$exception_index++])) {
        fail(sprintf
            "Didn't see expected exception of type [%s]",
            $extra->{ref},
        );
    }
}
...
sub is_deep_set {
    ...
    is_deeply_flex($got, $expect, $test_name) or print
...
This document describes version 0.02 of L<Data::Conveyor::App::Test::Stage>.
