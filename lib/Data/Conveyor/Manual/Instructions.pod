...
the data subobject. It then creates an instruction container for that person
...
subobjects which eventually have value objects) will have a kind of virtual
...
The transaction selector (txsel) for NICAT is also somewhat simplified. When
trying to determine whether there is a person.set-owner transaction, it only
has to ask the instruction container whether there is a change to the person's
name. Previously it had to read the old person object and check whether there
was a change.
