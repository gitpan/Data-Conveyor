...
sub apply_instruction_container {
    ...
    $self->data->apply_instruction_container(
        $self->instruction_container, $self->exception_container);
}
...
=head1 SYNOPSIS
    ...
    Data::Conveyor::Ticket::Payload::BusinessObject->new;
...
Data::Conveyor::Ticket::Payload::BusinessObject inherits from
L<Data::Conveyor::Ticket::Payload::Item>.

The superclass L<Data::Conveyor::Ticket::Payload::Item> defines these
methods and functions:

    DATA_PROPERTY(), clear_implicit(), data(), exception_container(),
    exception_container_clear(), exception_container_exists(),
    has_problematic_exceptions(), implicit(), implicit_clear(),
    implicit_set(), rc(), set_implicit(), status()
...
This document describes version 0.02 of L<Data::Conveyor::Ticket::Payload::BusinessObject>.
